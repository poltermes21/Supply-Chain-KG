...
EXPECTED_RELATIONSHIP_COUNTS = {
    'ORIGIN_FROM':      10_000,
    'DESTINATION_TO':   10_000,
    'SHIPPED_VIA':      10_000,
    'TRANSPORTS':       10_000,
    'USES_MODE':        10_000,
    'AFFECTED_BY':      10_000,
    'MITIGATED_WITH':   10_000,
    'HAS_RISK':         10_000,
    'LOCATED_IN':       11,
    'CONNECTS':         12,
    'VULNERABLE_TO':    7,
...
class TestRelationshipCounts:
    ...
    Tests:
        - Each relationship type has expected count
        - Total relationship count matches sum of individual counts
    """
    
    @pytest.mark.parametrize("rel_type,expected_count", EXPECTED_RELATIONSHIP_COUNTS.items())
    def test_relationship_count_by_type(self, neo4j_session, rel_type, expected_count):
        """
        Verify each relationship type has the expected count.
    ...
    def test_total_relationship_count(self, neo4j_session):
        """
        Verify total relationship count matches sum of expected counts.
        
        Notes:
            - Helps detect unexpected relationship types or duplicates
        """
        query = "MATCH ()-[r]->() RETURN count(r) as cnt"
        result = neo4j_session.run(query)
        actual_total = result.single()["cnt"]
        
        expected_total = sum(EXPECTED_RELATIONSHIP_COUNTS.values())
        
        assert actual_total == expected_total, (
            f"Expected {expected_total} total relationships, found {actual_total}"
        )
...
class TestDataIntegrity:
    ...
    def test_disruption_ids_valid_range(self, neo4j_session):
        """
        Verify all DisruptionType nodes have id in range [0, 3].
        ...
        assert invalid_count == 0, (
            f"Found {invalid_count} DisruptionType nodes with id outside [0, 3]"
        )
